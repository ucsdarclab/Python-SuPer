...
def depth_to_point_cloud(raft_disp, 
                         fx=882.0850326, 
                         fy=882.0850326, 
                         cx1=445.06146749, 
                         cy=190.24049547, 
                         cx2=445.061467, 
                         baseline=5.8513759749420302/1000.):
    
    depth = (fx * baseline) / (-raft_disp + (cx2 - cx1))
    _, _, H, W = depth.shape
    xx, yy = np.meshgrid(np.arange(W), np.arange(H))
    
    points_grid = np.stack(((xx-cx1)/fx, 
                            (yy-cy)/fy, 
                            np.ones_like(xx)
                           ), axis=0)
    points_grid = torch.from_numpy(points_grid)[None, :, :].to(depth.device) * depth

    return depth, points_grid.permute(0, 2, 3, 1)